...
parser.add_argument("--max_val_samples", type=int, default=None, help="Maximum number of validation samples to process. If None, process all samples.")
...
def _main(args):
    ...
    if args.test:
        ...
        # Limit the number of validation samples to process
    ...
    n_lines = []
    n_furnitures = []
    for val_id, val_example in tqdm(val_data.items(), total=len(val_data), desc=f'{args.gpt_type}'):
        # predict
        while True:
            if args.gpt_type == 'gpt3.5':
                prompt_for_gpt3,sorted_ids = form_prompt_for_gpt3(
